...
public class Gift implements java.io.Serializable{
    ...
    boolean debug = false;
		...
		private boolean thisBuckIsInBucks(){
				boolean ret = false;
				Connection con = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				String msg = "";
				String qq = " select count(*) from ebt_bucks where buck_id=? ";
				logger.debug(qq);
				try{
		...
		String addNewBuckToGift(){
				...
				if(buck_id.equals("") || id.equals("")){
						msg = "No buck number or gift pay id entered";
						return msg;
				}
				Buck buck = new Buck(debug, buck_id);
				//
		...
		String doCancelSelected(){
				...
				String qq2 = "update bucks b, gift_bucks gb set b.voided='y' where b.id=gb.buck_id and b.id=? and gb.gift_id=?";
				//
				logger.debug(qq);
